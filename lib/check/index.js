...
// Check is it Ionic project
check.isIonic = function isIonic(cache) {
    if (!fs.existsSync('ionic.config.json')) {
        console.log('\n');
        console.log(chalk.red('Couldn"t find ' + chalk.cyan('ionic.config.json') + ' file. Are you in an Ionic project?'));
        shell.exit(1);
    }
    return cache;
};
